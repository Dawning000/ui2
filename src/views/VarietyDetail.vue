...
import { fetchVarietyDetail, saveVariety, likeVariety, unlikeVariety, rateVariety, favoriteVariety, unfavoriteVariety, deleteVariety, type VarietySaveData, type VarietyRateData } from '@/api/varieties'
import type { VarietyDetail } from '@/types/varieties'
...
const error = ref('')
const detail = ref<VarietyDetail | null>(null)
const showEditForm = ref(false)
const likeLoading = ref(false)
const favoriteLoading = ref(false)
const ratingLoading = ref(false)
const deleteLoading = ref(false)
const userRating = ref<VarietyRateData>({
  score: 0,
  comment: ''
})
...
async function load() {
  ...
  error.value = ''
  try {
    detail.value = await fetchVarietyDetail(id)
    loaded.value = true
  } catch (err: any) {
    console.error('Failed to load variety detail:', err)
    if (err?.code === 404) {
      error.value = '综艺信息未找到'
    } else if (err?.message) {
...
// 处理表单提交
async function handleFormSubmit(data: VarietySaveData) {
  try {
    await saveVariety(data)
...
async function handleFavorite() {
  ...
  try {
    if (isFavorited.value) {
      // 取消收藏
      await unfavoriteVariety(id)
      detail.value.isFavorited = false
    } else {
      // 收藏
      await favoriteVariety(id)
...
async function handleSubmitRating() {
  ...
  try {
    await rateVariety(id, userRating.value)
    alert('评分提交成功！')
    // 重置评分表单
    userRating.value = {
      score: 0,
