...
const editFormData = computed(() => {
  ...
  return {
    ...
    awards: (detail.value.awards || []).map((a: any) => ({
      ...
      status: a.status || 'awarded',
      note: a.note || a.category || a.name || ''
    })),
    seasons: (detail.value.seasons || []).map((s: any) => ({
      id: s.id || 0,
      number: s.number || '1'
    })),
    episodes: detail.value.episodes || 1,
...
async function loadReviews() {
  ...
  try {
    const response = await fetchTvShowReviews(id, { 
...
.summary, .trailer-section, .photos-section, .seasons-section, .awards-section { 
  margin-top: 32px; 
  padding: 32px;
  background: var(--bg-card);
  border-radius: 16px;
...
.summary h2, .trailer-section h2, .photos-section h2, .seasons-section h2, .awards-section h2 { 
  font-size: 24px; 
  font-weight: 600; 
  color: var(--text-primary); 
  margin-bottom: 16px; 
